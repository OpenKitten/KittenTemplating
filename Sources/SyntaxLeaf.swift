...
public enum LeafSyntax: TemplatingSyntax {
    ...
    public static func compile(fromData input: [UInt8]) throws -> Template {
        return Template(compiled: try LeafSyntax.compile(fromData: input))
    }
    
    private static func compile(fromData input: [UInt8]) throws -> [UInt8] {
        var position = 0
        var rawBuffer = [UInt8]()
        ...
        func parseTag() throws -> [UInt8] {
            ...
            // "loop"
                ...
                let subTemplateCode = try LeafSyntax.compile(fromData: subTemplate) as [UInt8]
